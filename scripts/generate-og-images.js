...
async function generateOGImage({ title, author, date, backgroundImage, outputPath }) {
  ...
  try {
    ...
    console.warn(`Could not load background image: ${backgroundImage}`, error.message);
    ctx.fillStyle = "#0e7490";
    ctx.fillRect(0, 0, OG_WIDTH, OG_HEIGHT);
  }
      ...
      if (entry.isDirectory()) {
        ...
        if (fs.existsSync(indexPath)) {
          ...
          posts.push({
            ...
            author: data.author || "ReactJS.DE Team",
